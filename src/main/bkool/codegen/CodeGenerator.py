...
class CodeGenVisitor(BaseVisitor, Utils):
    ...
    def visitBinaryOp(self, ast, o):
        ...
        right_code, _ = ast.right.accept(self, o)
        """
        left_code: "bipush 10" (type str)
        right_code: "iconst_1" (type str)
        ...emitADDOP: "iadd" (type str)
        Can print out to justify: print(left_code), print(right_code)
        We have concat string so that when we visit (10 + 1) we will return a string of instructions
        ```bipush 10
        iconst_1
        iadd
        ```
        """
        if ast.op in ["+", "-"]:
            return left_code + right_code + self.emit.emitADDOP(ast.op, left_type, frame), left_type
        elif ast.op in ["*", "/"]:
            return left_code + right_code + self.emit.emitMULOP(ast.op, left_type, frame), left_type
    ...
    def visitFloatLiteral(self, ast, o):
        ...
        frame = ctxt.frame
        value = str(ast.value)
        return self.emit.emitPUSHFCONST(value, frame), FloatType()
